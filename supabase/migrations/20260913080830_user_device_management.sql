-- Authenticated users may list and revoke only their own OTHER devices.
-- Existing tables, credentials and grants are preserved; no server re-import.
begin;
...
begin
 ...
 if p_action like 'user_%' then
   ...
   if p_action='user_bootstrap' then return jsonb_build_object('account',yay_private.account(u.id),'servers',yay_private.catalog(),'current_device_id',d.id);end if;
   if p_action in('user_devices_list','user_devices_delete') then
     if p_action='user_devices_delete' then
       target=(p_payload->>'id')::uuid;
       if target=d.id then return '{"status":409,"error":"Use Log out on your current device."}'::jsonb;end if;
       -- The account lock above serializes removal with registration. Foreign-key
       -- cascade revokes every session for the removed device in this transaction.
       delete from yay_private.devices where id=target and user_id=u.id;
       if not found then return '{"status":404,"error":"Device not found on your account."}'::jsonb;end if;
     end if;
     select coalesce(jsonb_agg(jsonb_build_object('id',dv.id,'name',dv.name,
       'created_at',dv.created_at,'last_seen',dv.last_seen,'is_current',dv.id=d.id)
       order by (dv.id=d.id) desc,dv.created_at,dv.id),'[]'::jsonb)
       into a from yay_private.devices dv where dv.user_id=u.id;
     return jsonb_build_object('devices',a,'current_device_id',d.id,
       'account',yay_private.account(u.id),'free_slots',greatest(0,u.device_limit-jsonb_array_length(a)));
   end if;
