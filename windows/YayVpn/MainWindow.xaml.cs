...
public partial class MainWindow : Window {
    ...
    bool lightning,auto;Button? autoButton,lightningButton,enhancedButton;TextBlock? trafficText;
    ...
    void ConnectionModes(){
        autoButton=Add(T("Auto · all countries","自动 · 所有国家","Otomatis · semua negara"),()=>{
            if(busy||tunnel.Connected)return;auto=true;
            try{Directory.CreateDirectory(Path.GetDirectoryName(ModeFile)!);File.WriteAllText(ModeFile,"auto");}catch{}ShowHome();
        },auto);autoButton.IsEnabled=!busy&&!tunnel.Connected;

        var row=new Grid{Margin=new(0,0,0,8)};row.ColumnDefinitions.Add(new());row.ColumnDefinitions.Add(new());
        lightningButton=new Button{Content=new TextBlock{Text=T("Lightning connection","极速连接","Koneksi kilat"),TextWrapping=TextWrapping.Wrap,TextAlignment=TextAlignment.Center,FontSize=14,Foreground=lightning&&!auto?Brushes.Black:Brushes.White},Background=lightning&&!auto?Red:new SolidColorBrush(Color.FromRgb(25,27,32)),Foreground=lightning&&!auto?Brushes.Black:Brushes.White,IsEnabled=!busy&&!tunnel.Connected,Margin=new(0,0,5,0)};
        enhancedButton=new Button{Content=new TextBlock{Text=T("Enhanced connection","优选连接","Koneksi optimal"),TextWrapping=TextWrapping.Wrap,TextAlignment=TextAlignment.Center,FontSize=14,Foreground=!lightning&&!auto?Brushes.Black:Brushes.White},Background=!lightning&&!auto?Red:new SolidColorBrush(Color.FromRgb(25,27,32)),Foreground=!lightning&&!auto?Brushes.Black:Brushes.White,IsEnabled=!busy&&!tunnel.Connected,Margin=new(5,0,0,0)};
        ...
        Root.Children.Add(new TextBlock{Text=auto?T("Tests VPN routes across countries in pools of 12. Chooses the lowest measured delay and rechecks every 30 seconds. First connection may take longer.","每组测试12条跨国 VPN 线路，选择实测低延迟线路，每30秒复测。首次连接可能较慢。","Menguji rute VPN lintas negara per 12 server. Memilih latensi terendah dan menguji ulang tiap 30 detik. Koneksi awal bisa lebih lama."):lightning?T("Random server in your country. Skips the ping scan.","随机选择当前国家的服务器，跳过延迟扫描。","Server acak di negara pilihan. Tanpa pemindaian ping."):T("Tests for up to 12 seconds, then uses the fastest measured options.","最多测试12秒，然后选择实测较快的服务器。","Menguji hingga 12 detik, lalu memilih server terukur tercepat."),FontSize=13,Foreground=Brushes.Gray,TextAlignment=TextAlignment.Center,TextWrapping=TextWrapping.Wrap,Margin=new(0,4,0,18)});
    }
    void SetMode(bool fast){if(busy||tunnel.Connected)return;auto=false;lightning=fast;try{Directory.CreateDirectory(Path.GetDirectoryName(ModeFile)!);File.WriteAllText(ModeFile,fast?"lightning":"enhanced");}catch{}ShowHome();}
    ...
    async Task ConnectCountry(string country,CancellationToken ct){
        if(auto){await ConnectAutomatic(ct);return;}
    ...
    async Task ConnectAutomatic(CancellationToken ct){
        ...
        }}catch(OperationCanceledException) when(!ct.IsCancellationRequested){throw new TimeoutException("Auto search timed out. No working route was confirmed in two minutes. Try another network or contact support.");}
        throw new IOException("Auto could not confirm a working server. Refresh locations or try another network.",last);
    }
    ...
    async Task Run(Func<CancellationToken,Task> action){
        ...
        if(autoButton!=null)autoButton.IsEnabled=false;if(picker!=null)picker.IsEnabled=false;if(lightningButton!=null)lightningButton.IsEnabled=false;if(enhancedButton!=null)enhancedButton.IsEnabled=false;
        if(status!=null)status.Text=auto?T("Auto: testing VPN routes… Tap to cancel.","自动：测试 VPN 线路…点击取消。","Otomatis: menguji rute VPN… Ketuk untuk batal."):lightning?T("Connecting… Tap to cancel.","正在连接…点击取消。","Menghubungkan… Ketuk untuk batal."):T("Testing for up to 12 seconds. Tap to cancel.","最多测试12秒。点击取消。","Menguji hingga 12 detik. Ketuk untuk batal.");
